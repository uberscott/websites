...
var Advertiser =
{
   ...
   {
      "DisplayedFriendEUD" : "",
      "PixelSrc" : "https://web.archive.org/web/20071003193406/http://seg.fimserve.com/relay?"
   },
   "DC" :
   {
      "AdSource" : ""
   ...
   "CMS" :
   {
      "BlueLithium" : "myspace_bluelithium"
   }
}
...
{
   regEx = new RegExp(key+'=([^&]*)','i');
   if (decodeURI(document.URL).search(regEx) == -1)
   {
      return null;
   }
   return RegExp.$1;
...
Advertiser.Util.AddEvent = function(obj, evType, fn)
{
...
Advertiser.SDC.CheckBlueLith = function(blcode)
{
	if(blcode == null || blcode.length == 0 || blcode.indexOf('//web.archive.org/web/20071003193406/http://no Content') > -1)
		Advertiser.SDC.WritePixel();
	else
	{
      eval(blcode);
		setTimeout('Advertiser.SDC.WritePixel();', 1000);
	}	
}

Advertiser.SDC.RegisterPartners = function()
{
    try
    {
        MySpace.CMS.getContent(Advertiser.CMS.BlueLithium, Advertiser.SDC.CheckBlueLith);
    }
    catch(e)
    {
        Advertiser.SDC.WritePixel();
...
Advertiser.SDC.WritePixel = function()
{
...
Advertiser.SDC.DropPixel = function ()
{
    ...
    if (decodeURI(document.URL).search(regex) != -1 && RegExp.$1 == 'ad')	
        return;
   
	if (Advertiser.Login.IsLogin())
    {
	    var added = Advertiser.Util.AddEvent(window, 'load', Advertiser.SDC.RegisterPartners);
        if(!added)
            Advertiser.SDC.WritePixel();
	}
}
...
oas_helper.getMySpaceTVHomeCategory = function () {
	if(document.URL=="https://web.archive.org/web/20071003193406/http://vids.myspace.com" || document.URL=="https://web.archive.org/web/20071003193406/http://vids.myspace.com/")
	{
	return "1";
	}
	else
	{
	    var qs_action = oas_helper.getID('fuseaction');
	    var qs_placement = oas_helper.getID('placement');
...
function sdc_wrapper()
{
    ...
    var tvhcat_id = oas_helper.getMySpaceTVHomeCategory();
   ...
   { 
      ...
      if (runBandGenreAd)
      {
         frameURL = "http://"+subd+".opt.fimserve.com/adopt/?l="+page+"&pos=" + pos + "&r=h&rnd="+rand + friendID + SchoolID + AdTopicID + videoID + bg1 + bg2 + bg3 + downloadCat + tvcatmaster_id + tvvideo_id + tvcat_id + tvchan_id + tvhcat_id + testvalue + "&ged=" + ged;
      }
      else
      {
         frameURL = "http://"+subd+".opt.fimserve.com/adopt/?l="+page+"&pos=" + pos + "&r=h&rnd="+rand + friendID + SchoolID + AdTopicID  + videoID+ downloadCat + tvcatmaster_id + tvvideo_id + tvcat_id + tvchan_id + tvhcat_id + testvalue + "&ged=" + ged;
      }
...
function ad_wrapper()
{
	...
	{
		if(testmode)
			frameURL = "https://web.archive.org/web/20071003193406/http://detst.myspace.com/html.ng/site=myspace&position="+pos+"&page="+page+"&rand="+rand+friendID+AdTopicID+"&acnt="+oas_helper.adCount+"&schoolpage="+SchoolID+bg1+bg2+bg3+"&special="+special+videoID+videoUserCat+videoChannel+downloadCat+tvcatmaster_id+tvvideo_id+tvcat_id+tvchan_id+tvhcat_id;
		else
			frameURL = "http://"+subd+".myspace.com/html.ng/site=myspace&position="+pos+"&page="+page+"&rand="+rand+friendID+AdTopicID+"&acnt="+oas_helper.adCount+"&schoolpage="+SchoolID+bg1+bg2+bg3+videoID+videoUserCat+videoChannel+downloadCat+tvcatmaster_id+tvvideo_id+tvcat_id+tvchan_id+tvhcat_id;
	}
	...
	{
		if (testmode)
			frameURL = "https://web.archive.org/web/20071003193406/http://detst.myspace.com/html.ng/site=myspace&position="+pos+"&page="+page+"&rand="+rand+friendID+AdTopicID+"&acnt="+oas_helper.adCount+"&schoolpage="+SchoolID+"&special="+special+videoID+videoUserCat+videoChannel+downloadCat+tvcatmaster_id+tvvideo_id+tvcat_id+tvchan_id+tvhcat_id;
		else 
			frameURL = "http://"+subd+".myspace.com/html.ng/site=myspace&position="+pos+"&page="+page+"&rand="+rand+friendID+AdTopicID+"&acnt="+oas_helper.adCount+"&schoolpage="+SchoolID+videoID+videoUserCat+videoChannel+downloadCat+tvcatmaster_id+tvvideo_id+tvcat_id+tvchan_id+tvhcat_id;
	}
...
/*
     FILE ARCHIVED ON 19:34:06 Oct 03, 2007 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 19:05:44 Jun 02, 2025.
...
playback timings (ms):
  ...
  esindex: 0.013
  cdx.remote: 34.042
  LoadShardBlock: 148.184 (3)
  PetaboxLoader3.datanode: 242.706 (4)
  load_resource: 265.592
  PetaboxLoader3.resolve: 120.79
*/
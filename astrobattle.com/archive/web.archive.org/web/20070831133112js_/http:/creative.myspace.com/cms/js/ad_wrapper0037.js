...
Advertiser.SDC.DropPixel = function ()
{
   ...
   if (Advertiser.Login.IsLogin())
   {
      ...
      fimpixel = '<img src="https://web.archive.org/web/20070831141228/http://seg.fimserve.com/relay?';
...
function sdc_wrapper()
{
   ...
   { 
      ...
      if (runBandGenreAd)
      {
         frameURL = "http://"+subd+".opt.fimserve.com/adopt/?l="+page+"&pos=" + pos + "&r=h&rnd="+rand + friendID + SchoolID + AdTopicID + videoID + bg1 + bg2 + bg3 + downloadCat + tvcatmaster_id + tvvideo_id + tvcat_id + tvchan_id + "&ged=" + ged;
      }
      else
      {
         frameURL = "http://"+subd+".opt.fimserve.com/adopt/?l="+page+"&pos=" + pos + "&r=h&rnd="+rand + friendID + SchoolID + AdTopicID  + videoID+ downloadCat + tvcatmaster_id + tvvideo_id + tvcat_id + tvchan_id + "&ged=" + ged;
      }
...
function ad_wrapper()
{
	...
	{
		if(testmode)
			frameURL = "https://web.archive.org/web/20070831141228/http://detst.myspace.com/html.ng/site=myspace&position="+pos+"&page="+page+"&rand="+rand+friendID+AdTopicID+"&acnt="+oas_helper.adCount+"&schoolpage="+SchoolID+bg1+bg2+bg3+"&special="+special+videoID+videoUserCat+videoChannel+downloadCat+tvcatmaster_id+tvvideo_id+tvcat_id+tvchan_id;
		else
	...
	{
		if (testmode)
			frameURL = "https://web.archive.org/web/20070831141228/http://detst.myspace.com/html.ng/site=myspace&position="+pos+"&page="+page+"&rand="+rand+friendID+AdTopicID+"&acnt="+oas_helper.adCount+"&schoolpage="+SchoolID+"&special="+special+videoID+videoUserCat+videoChannel+downloadCat+tvcatmaster_id+tvvideo_id+tvcat_id+tvchan_id;
		else 
...
/*
     FILE ARCHIVED ON 14:12:28 Aug 31, 2007 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 15:36:19 Jun 02, 2025.
...
playback timings (ms):
  ...
  esindex: 0.034
  cdx.remote: 78.256
  LoadShardBlock: 196.742 (3)
  PetaboxLoader3.resolve: 149.912 (3)
  PetaboxLoader3.datanode: 143.591 (4)
  load_resource: 134.245
*/